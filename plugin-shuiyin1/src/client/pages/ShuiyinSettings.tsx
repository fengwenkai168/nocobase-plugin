...
import { Card, Form, Input, InputNumber, Select, Switch, Button, Space, message } from 'antd';
  ...
  const { loading } = useRequest(
    { url: 'shuiyin1_settings:list' },
    {
      onSuccess: (data) => {
        ...
        if (record) {
          if (!Array.isArray(record.textSources) || !record.textSources.length) {
  ...
  return (
    <Card title={t('Watermark Settings')} loading={loading}>
      <Form form={form} layout="vertical" style={{ maxWidth: 600 }}>
        ...
        <Form.Item label={t('Opacity')} name="opacity" rules={[{ required: true, message: t('Please enter opacity') }]}>
        ...
        <Form.Item label={t('Density')} name="density" rules={[{ required: true, message: t('Please enter density') }]}>
