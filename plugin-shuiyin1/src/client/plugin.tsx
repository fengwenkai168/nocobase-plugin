...
export class PluginShuiyin1Client extends Plugin {
  ...
  private async refreshWatermark(reason?: string, nextSettings?: Required<WatermarkSettings>) {
    this.settings = nextSettings ? { ...defaultSettings, ...nextSettings } : await this.fetchSettings();
    if (reason) {
  ...
  async load() {
    ...
    // Fetch current user
    try {
      const userRes = await this.app.apiClient.request({ url: 'auth:check', method: 'GET' });
