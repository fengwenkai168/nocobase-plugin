...
- **优化（导出步骤2 字段排序合并为一组 + 全局连续序号）**：原按 4 个分组（常规/日期/关联/附件）分别渲染、每组各自从 1 编号，字段多时"到处都是 123"且无法看出全局列顺序。现合并为**单一排序列表**：
  - 所有已选字段按全局顺序（`selectedFields`）渲染，**序号 = 最终导出列顺序**（跨类型连续），拖拽/上下移/序号跳转即时生效；
  - 每行字段名旁显示类型 Tag（常规/日期/关联/附件）替代分组标题；
  - 日期字段行内日期格式 Select、关联字段行内关联格式 Select 保留（按字段类型自动判断）；
  - 底部统一「+ 添加字段」下拉（未选字段平铺 + 类型 Tag）；
  - 跨类型字段（如日期字段插在常规字段中间）自然支持，不再受分组限制。
- `SortableExportRow` 的 `label` 类型放宽为 `ReactNode` 以支持行内 Tag。
