...
export default observer(function ExportPanel() {
  ...
  React.useEffect(() => {
    const loadScopes = async () => {
      ...
      try {
        const res = await apiRequest(client, 'sjgl02Permissions:scopes', { data: params });
  ...
  React.useEffect(() => {
    const filterOptions = async () => {
      ...
      setPermSourceOptions(filtered);
    };
    filterOptions();
  }, [selTable, isAdminOrRoot]);

  const handlePermSourceChange = async (val: string) => {
    if (val === 'admin') {
      setPermSource({ type: 'admin', label: '管理员完整权限' });
      setPermExportFields([]);
      setCurrentUserFilter(null);
      setScopeSourceLabel('管理员完整权限');
      return;
    }
    ...
    try {
      const permData = await apiRequest(client, 'sjgl02Permissions:get', {
        params: { targetType: type, targetId: id },
      });
  ...
  React.useEffect(() => {
    if (selTable && selTable !== '__all__') {
      client
        .request({ url: 'sjgl02Export:tableFields', method: 'get', params: { tableName: selTable } })
        .then((res: any) => {
          const fArr = res?.data?.data || [];
          if (!Array.isArray(fArr)) return;
          const allFields = fArr.map((f: any) => ({ ...f, displayName: f.name }));
          apiRequest(client, 'auth:check')
            .then((userData: any) => {
              const uid = userData?.data?.id || userData?.id;
              const roles = (userData?.roles || userData?.data?.roles || []).map((r: any) => r.name || '');
              if (roles.includes('admin') || roles.includes('root')) {
                ...
                apiRequest(client, 'sjgl02Permissions:get', {
                  params: { targetType: permSource.type, targetId: permSource.id },
                })
                  .then((permData: any) => {
                    const perm = (permData?.custom || [])
              ...
              apiRequest(client, 'sjgl02Permissions:get', { params: { targetType: 'user', targetId: String(uid) } })
                .then((permData: any) => {
      ...
      client
        .request({ url: 'sjgl02Export:previewCount', method: 'post', data: { tableName: '__all__' } })
        .then((res: any) => {
          const c = res?.data?.data?.estimatedRows;
          if (typeof c === 'number') setEstimatedRows(c);
        })
  ...
  const selTableCollection =
    selTable && selTable !== '__all__' ? app?.dataSourceManager?.getCollection?.('main', selTable) : undefined;

  const toggleField = (name: string) =>
    setSelFields((p) => (p.includes(name) ? p.filter((f) => f !== name) : [...p, name]));
      ...
      {step === 0 && (
        <div>
          ...
          <div style={{ textAlign: 'right', marginTop: 12 }}>
            <Button type="primary" disabled={!selTable} onClick={() => setStep(1)}>
              下一步 →
            </Button>
          </div>
        </div>
      ...
      {step === 1 && (
        <div>
            ...
            <Card size="small" style={{ marginBottom: 16, backgroundColor: '#f0f7ff', border: '1px solid #bae0ff' }}>
              <Space>
                ...
                <Select
                  ...
                  onChange={handlePermSourceChange}
                  style={{ minWidth: 240 }}
                  size="small"
          ...
          {isAllTables ? (
            ...
            <>
              <Card title="☑️ 字段选择" size="small" style={{ marginBottom: 16 }}>
                <div style={{ marginBottom: 12, paddingBottom: 12, borderBottom: '1px solid #f0f0f0' }}>
                  <Checkbox
                    ...
                    onChange={() =>
                      selFields.length === fields.length ? setSelFields([]) : setSelFields(fields.map((f) => f.name))
                    }
                  >
                    全选{' '}
                  ...
                  <>
                    ...
                    <Space wrap style={{ marginBottom: 12 }}>
                      {regular.map((f) => (
                        <Checkbox
                          key={f.name}
                          checked={selFields.includes(f.name)}
            ...
            <Card title="📊 数据范围" size="small" style={{ marginBottom: 12 }} loading={loadingScopes}>
              ...
              {!selTableCollection ? (
                <div style={{ textAlign: 'center', padding: 20 }}>
                  <Spin size="small" /> <span style={{ marginLeft: 8, color: '#999' }}>加载数据表集合...</span>
                </div>
                ...
                <>
                  <Alert
                    type="warning"
                    showIcon
                    message="当前方案已限定导出范围，不可修改"
                    style={{ marginBottom: 10 }}
                  />
                  <div
                    style={{ pointerEvents: 'none', opacity: 0.85, background: '#f8fafc', padding: 8, borderRadius: 4 }}
                  >
          ...
          <div style={{ textAlign: 'right', marginTop: 16 }}>
            ...
            <Button
              type="primary"
              onClick={() => setStep(2)}
              disabled={!selTable || (!isAllTables && selFields.length === 0)}
            >
              下一步 →
            </Button>
          </div>
      ...
      {step === 2 && (
        <div>
          <Row gutter={12} style={{ marginBottom: 16 }}>
            <Col span={6}>
              <Card size="small">
                <Statistic title="选择字段" value={isAllTables ? '全部' : selFields.length} />
              </Card>
            </Col>
            <Col span={6}>
              <Card size="small">
