...
export default observer(function PermissionTab() {
  ...
  const userTargets = targets.filter(
    (t: any) =>
      t.type === 'user' && (!searchText || (t.nickname || '').toLowerCase().includes(searchText.toLowerCase())),
  );
  const roleTargets = targets.filter(
    (t: any) =>
      t.type === 'role' && (!searchText || (t.nickname || '').toLowerCase().includes(searchText.toLowerCase())),
  );
  ...
  const handleSave = async () => {
    try {
      ...
      const ok = await save(values, editModal.perm);
      if (ok) {
        message.success(t('Saved successfully'));
        setEditModal({ open: false });
      } else message.error(t('Save failed'));
    } catch {
  ...
  const auditColumns = [
    ...
    { title: '数据表', dataIndex: 'tableName', width: 100 },
    {
      title: '变更概要',
      dataIndex: 'changes',
      render: (v: any) => (v ? (v.after && !v.before ? '新增权限' : !v.after ? '移除权限' : '修改配置') : '—'),
    },
  ];
    ...
    >
      ...
      <Space wrap size={[6, 6]}>
        <Tag color={perm.canImport ? 'blue' : 'default'}>
          {perm.canImport ? '✅' : '⛔'} 导入: {perm.canImport ? '允许' : '不允许'}
        </Tag>
        <Tag color={perm.canExport ? 'green' : 'default'}>
          {perm.canExport ? '✅' : '⛔'} 导出: {perm.canExport ? '允许' : '不允许'}
        </Tag>
        {perm.canImport && (
          <>
            <Tag color="orange">
              ...
              {(Array.isArray(perm.importMode) ? perm.importMode : [perm.importMode || 'insert'])
                .map((m: string) => ({ insert: '新增', update: '更新', upsert: '新增+更新' })[m] || m)
                .join(' / ')}
            </Tag>
        ...
        {perm.canImport && (
          <Tag color="cyan">可导入: {perm.importFields?.length > 0 ? perm.importFields.length + '个字段' : '全部'}</Tag>
        )}
        {perm.canExport && (
          <Tag color="purple">
  ...
  return (
    <div>
                    ...
                    {group.items.map((item: any) => (
                      <div
                        ...
                        style={{
                          ...
                          marginBottom: 2,
                          background:
                            selectedTarget?.id === item.id && selectedTarget?.type === item.type
                              ? '#e6f4ff'
                              : undefined,
                          color:
                            selectedTarget?.id === item.id && selectedTarget?.type === item.type
                ...
                <div>
                  {selectedTarget?.type === 'role' && isSystemManaged && (
                    <Alert
                      type="info"
                      showIcon
                      message="管理员/超级管理员角色拥有全部数据表权限，不可修改"
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                    {!isSystemManaged && customPerms.length > 0 && (
                      <>
                        ...
                        {selectedRows.size > 0 && (
                          <Popconfirm
                            title={`确认删除选中的 ${selectedRows.size} 条权限？`}
                            onConfirm={() => {
                            ...
                            {filter.inheritedPerms.length > 0 && (
                              <div style={{ marginBottom: 10 }}>
                                ...
                                >
                                  ...
                                  角色继承的权限（{filter.inheritedPerms.length}条）
                                </div>
                            ...
                            {filter.customPerms.length > 0 && (
                              <div style={{ marginBottom: 10 }}>
                                ...
                                >
                                  ...
                                  用户自定义权限（{filter.customPerms.length}条）
                                </div>
        ...
        <Form form={form} layout="vertical">
          <Form.Item label={t('Select table')} name="tableName" rules={[{ required: true }]}>
            <Select
              ...
              options={tables
                .filter((t: any) => {
                  if (editModal.perm && editModal.perm.tableName === t.name) return true;
            ...
            <>
              ...
              {formMode.some((m) => m === 'update' || m === 'upsert') && (
                <Form.Item
                  label="唯一值字段"
                  name="uniqueFields"
                  rules={[{ required: true, message: '更新/新增+更新模式必须配置唯一值字段' }]}
                >
                  <Select
                    mode="multiple"
                    showSearch
      ...
      <Modal
        ...
        footer={<Button onClick={() => setDetailModal({ open: false })}>{t('Close')}</Button>}
        width={680}
      >
        {detailModal.perm && (
          <div>
            ...
            <Descriptions column={1} size="small" bordered>
              <Descriptions.Item label="数据表">
                <b>{detailModal.perm.tableName}</b>
              </Descriptions.Item>
              <Descriptions.Item label="允许导入">
                {detailModal.perm.canImport ? <Tag color="blue">是</Tag> : <Tag>否</Tag>}
              </Descriptions.Item>
              <Descriptions.Item label="允许导出">
                {detailModal.perm.canExport ? <Tag color="blue">是</Tag> : <Tag>否</Tag>}
              </Descriptions.Item>
              <Descriptions.Item label="导入模式">
                {(Array.isArray(detailModal.perm.importMode)
                  ? detailModal.perm.importMode
                  : [detailModal.perm.importMode || 'insert']
                ).map((m: string) => (
                  <Tag key={m} color="orange">
                    {m === 'insert' ? '新增' : m === 'update' ? '更新' : '新增+更新'}
                  </Tag>
                ))}
              </Descriptions.Item>
              <Descriptions.Item label="唯一值字段">
                {detailModal.perm.uniqueFields?.length > 0 ? detailModal.perm.uniqueFields.join(', ') : '—'}
              </Descriptions.Item>
              <Descriptions.Item label="必填字段">
                {detailModal.perm.requiredFields?.length > 0 ? detailModal.perm.requiredFields.join(', ') : '—'}
              </Descriptions.Item>
