...
function CardWrap({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <Card title={title} size="small" style={{ marginBottom: 8, borderRadius: 8 }}
...
function CollapseCard({ title, defaultOpen, children }: { title: string; defaultOpen?: boolean; children: React.ReactNode }) {
  const [open, setOpen] = useState(defaultOpen !== false);
  return (
    <Card size="small" style={{ marginBottom: 8, borderRadius: 8 }}
      styles={{ header: { fontSize: 13, fontWeight: 600, minHeight: 36 } }}
      title={<span style={{ cursor: 'pointer' }} onClick={() => setOpen(!open)}>
...
export function TaskSummaryCard({ task, api, tableTitles, fieldTitles }: any) {
  ...
  const getAttachmentUrl = async (): Promise<{ url: string; filename: string }> => {
    ...
    try {
      ...
      return { url, filename };
    } catch { return { url: '', filename: '' }; }
  };

  const previewFile = async () => {
    const { url } = await getAttachmentUrl();
    if (!url) { message.warning('无法获取文件预览地址'); return; }
  ...
  const downloadFile = async () => {
    ...
    try {
      ...
      if (!url) { message.error('无法获取文件地址'); return; }
  ...
  const failedCount = Array.isArray(task.errorLogs) ? task.errorLogs.length : Math.max(0, (task.totalRows || 0) - (task.processedRows || 0));
  ...
  return (
    <CardWrap title={`${isImport ? '📥 导入任务' : '📤 导出任务'} · 摘要`}>
      ...
      <Descriptions column={2} size="small" style={{ marginTop: 8 }}>
        ...
        <Descriptions.Item label={isImport ? '完成时间' : '完成时间'}>
      ...
      {task.status === 'processing' && task.totalRows > 0 && (() => {
        ...
        return (
          <div style={{ textAlign: 'center', color: '#6b7280', fontSize: 12, marginTop: 8 }}>
            预计剩余时间：约 {remaining > 60 ? `${Math.floor(remaining / 60)} 分 ${remaining % 60} 秒` : `${remaining} 秒`}
          </div>
        );
      })()}
      ...
      {task.status === 'processing' && (
        <div style={{ textAlign: 'center', marginTop: 12 }}>
          <Button size="small" danger onClick={() => {
            modal.confirm({ title: '确认取消', content: `确定要取消任务 #${task.id} 吗？`, okText: '确认取消', okButtonProps: { danger: true },
              onOk: async () => {
                try {
...
export function ExportFieldsCard({ task, fieldTitles, tableTitles, assocFieldTitles, assocFieldMap }: any) {
  ...
  if (mainFields.length > 0) {
    tabItems.push({
      key: 'main',
      label: '主表字段',
      children: <Space wrap>{mainFields.map((f: string) => <FieldTag key={f} name={f} title={fieldTitles[f]} />)}</Space>,
    });
  }
  assocTables.forEach((assocTable: string) => {
    ...
    tabItems.push({
      ...
      children: fieldEntries.length > 0
        ? <Space wrap>{fieldEntries.map(([name, title]) => <FieldTag key={name} name={name} title={title} />)}</Space>
...
export function RelationTablesCard({ task, tableTitles, assocFieldMap, api }: any) {
  if (task.taskType !== 'export' || !task.includeAssociationSheet || !task.associationSheetTables?.length) return null;
  ...
  useEffect(() => {
    const loadCounts = async () => {
      ...
      await Promise.all(tables.map(async (fieldName: string) => {
        const info = assocFieldMap?.[fieldName];
        const targetTable = info?.targetTable;
        if (!targetTable) { newCounts[fieldName] = -1; return; }
        try {
  ...
  return (
    <CardWrap title="关联表导出详情">
      <Table dataSource={tables.map((t: string, i: number) => ({ key: i, fieldName: t }))} pagination={false} size="small"
        columns={[
          { title: 'Sheet名称', render: (_: any, r: any) => {
            const info = assocFieldMap?.[r.fieldName];
            return <span style={{ fontFamily: 'monospace', fontSize: 13, color: '#059669' }}>{info ? `${info.fieldName}-${info.targetTable || r.fieldName}` : r.fieldName}</span>;
          }},
          { title: '关联表', render: (_: any, r: any) => {
            const info = assocFieldMap?.[r.fieldName];
            const tblName = info?.targetTable || r.fieldName;
            return <TableTag name={tblName} title={tableTitles?.[tblName]} />;
          }},
          { title: '数据量', render: (_: any, r: any) => {
      ...
      <Descriptions column={1} size="small">
        ...
        <Descriptions.Item label="唯一值字段">
          {uniqueFields.length > 0 ? (
            <Space wrap>{uniqueFields.map((f: string) => <span key={f} style={{ color: '#f59e0b' }}>⭐ <FieldTag name={f} title={fieldTitles[f]} /></span>)}</Space>
...
export function FieldMappingCard({ task, fieldTitles }: any) {
  ...
  return (
    <CollapseCard title="字段映射" defaultOpen={false}>
      <Table dataSource={displayEntries.map(([k, v]: any, i: number) => {
        const isUnique = uniqueFields.includes(k);
        const isCustom = v === '__custom__';
        const isIgnored = v === '__ignore__';
        let mapType: string;
        let excelCol: string;
        if (isIgnored) { mapType = '忽略'; excelCol = '—'; }
        else if (isCustom) { mapType = '自定义'; excelCol = (task.customValues || {})[k] || '自定义内容'; }
        else { mapType = 'Excel列'; excelCol = v; }
        let label = '';
        ...
        columns={[
          ...
          { title: '映射方式', dataIndex: 'mapType', width: 90, render: (v: string) => {
          ...
          { title: '标签', dataIndex: 'label', width: 100, render: (v: string) => {
            if (!v) return '—';
            return v.split(' · ').map((s: string) => {
              if (s.includes('唯一值')) return <Tag key={s} color="#f59e0b" style={{ fontSize: 11 }}>⭐ 唯一值</Tag>;
              return null;
            });
          }},
        ]} />
      {entries.filter(([, v]: any) => v === '__custom__').length > 0 && (
        <div style={{ color: '#6b7280', fontSize: 12, marginTop: 8 }}>
          自定义值：{entries.filter(([, v]: any) => v === '__custom__').map(([k]: any) => `${(task.customValues || {})[k] || '（未知）'} → ${fieldTitles[k] || k}(${k})`).join('、')}
        </div>
      )}
    </CollapseCard>
  );
