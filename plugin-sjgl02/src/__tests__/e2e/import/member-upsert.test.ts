...
test.describe('普通用户 upsert 导入', () => {
  ...
  test('普通用户在区块中完成 upsert 导入', async ({ page, adminApi }) => {
    ...
    await uploadFixture(page, 'member-upsert.xlsx');
    await expect(page.getByText('member-upsert.xlsx')).toBeVisible();
    ...
    expect(records.find((r) => r.code === 'A001')?.price).toBe(88);
    expect(records.find((r) => r.code === 'A002')?.name).toBe('新商品');
  });
});
