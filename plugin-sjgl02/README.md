...
- **⬆ 数据导出**：字段级勾选导出，已选字段合并为一个统一排序列表（全局连续序号=导出列顺序，拖拽/序号/上下移动，行内类型标签与日期/关联格式配置），「复用其他方案排序」一键套用其他权限方案的字段顺序，关联表独立 Sheet 或独立文件、附件打包 tar.gz、百万行自动分表、自定义筛选、全部数据表导出（admin/root）。
