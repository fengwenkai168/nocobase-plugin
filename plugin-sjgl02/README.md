...
- **⬆ 数据导出**：字段级勾选导出（常规/日期/关联/附件分组），进入步骤2 默认按权限配置的字段顺序排序，拖拽/序号/上下移动可改（顺序即导出列顺序），「复用其他方案排序」一键套用其他权限方案的字段顺序，关联表独立 Sheet 或独立文件、附件打包 tar.gz、百万行自动分表、自定义筛选、全部数据表导出（admin/root）。
